...
function getFilteredLocations() {
    fetch('/places').then(response => response.json()).then(data => {
        let rObj = new Set();
...
function getWeatherResult() {
    ...
    fetch(`/places/${selection}`).
    then(response => response.json()).
    then(data => {

        let time = new Date;
        let currentDate = `${time.getFullYear()}-${getDoubleDigit(time.getMonth() + 1)}-${getDoubleDigit(time.getDate())}`;
